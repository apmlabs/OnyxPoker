...
class HelperBar:
    def __init__(self):
        ...
        # Log level filter
        self.log_level = "ALL"  # ALL, INFO, DEBUG, ERROR
    ...
    def create_ui(self):
        ...
        # Log filter buttons
        filter_frame = tk.Frame(center, bg='#1a1a1a')
        filter_frame.pack(fill='x')
        
        self.filter_buttons = {}
        for level in ["ALL", "INFO", "DEBUG", "ERROR"]:
            btn = tk.Button(filter_frame, text=level, font=('Arial', 8),
                           bg='#333' if level != "ALL" else '#555', fg='#fff',
                           relief='flat', padx=8, pady=2,
                           command=lambda l=level: self.set_log_filter(l))
            btn.pack(side='left', padx=1)
            self.filter_buttons[level] = btn
        
        tk.Button(filter_frame, text="Clear", font=('Arial', 8),
                 bg='#333', fg='#fff', relief='flat', padx=8, pady=2,
                 command=self.clear_log).pack(side='right', padx=1)
        
        tk.Button(filter_frame, text="Copy", font=('Arial', 8),
                 bg='#333', fg='#fff', relief='flat', padx=8, pady=2,
    ...
    def set_log_filter(self, level):
        ...
        for l, btn in self.filter_buttons.items():
            btn.config(bg='#555' if l == level else '#333')
    ...
    def register_hotkeys(self):
        ...
        self.log("Hotkeys registered: F9-F12", "DEBUG")
    ...
    def _analyze_thread(self):
        ...
        try:
            ...
            self.log(f"Screenshot: {active.title[:30]}... ({active.width}x{active.height})", "DEBUG")
            ...
            try:
                # AI analysis
                self.log(f"API call ({MODEL})...", "DEBUG")
                vision = VisionDetector(logger=lambda m, l="DEBUG": self.log(m, l))
    ...
    def _display_result(self, result, elapsed, screenshot):
        ...
        self.reasoning_label.config(text=reasoning[:150] if reasoning else "")
        
        self.conf_label.config(text=f"Conf: {confidence:.0%}")
    ...
    def _update_thumbnail(self, img):
        ...
        try:
            ...
            self.thumb_canvas.create_image(140, 40, image=self.thumb_photo)
        except Exception as e:
    ...
    def on_f10(self):
        ...
        if self.bot_running:
            ...
            self.log("Bot started (not implemented yet)", "INFO")
